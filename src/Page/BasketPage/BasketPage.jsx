...
const ACTIVE_CTX_KEY = "activeContext";
...
function BasketPage() {
  ...
  const [roomOrderMode, setRoomOrderMode] = useState(() =>
    readBooleanLS("roomOrder", false)
  );
  ...
  useEffect(() => {
    ...
    const onStorage = (e) => {
      ...
      if (e.key === "roomOrder")
        setRoomOrderMode(readBooleanLS("roomOrder", false));
  ...
  useEffect(() => {
    ...
    const others = scored
      .filter((x) => !usedIds.has(x.product.id))
      .slice(0, OTHER_LIMIT)
      .map((x) => x.product);
  ...
  const handleConfirm = async () => {
    try {
      ...
      if (roomOrderMode) {
        if (!c?.roomNumber) {
          alert(
            "Otaq nömrəsi tapılmadı (customerData.roomNumber). Xahiş olunur, məlumatları doldurun."
          );
          return;
        }
        ...
        if (!hotelRoomId) {
          alert(
            "Hotel otaq backend-də tapılmadı. Xahiş olunur, nömrəni yoxlayın."
          );
          return;
        }
      ...
      if (!kind || !number) {
        alert(
          "Masa/Otaq nömrəsi tapılmadı. Zəhmət olmasa NFC linkindən daxil olun."
        );
        return;
      }
      ...
      if (kind === "table") {
        ...
        await axios.post(TABLE_BASKETS_POST_URL, payload, {
          headers: { "Content-Type": "application/json" },
        });

        try {
          await updateTableStatus(backendId, "waitingFood");
        } catch {}
      }
  ...
  const contextLabel = displayIsTable
    ? "Masa"
    : displayIsRoom
    ? "Otaq"
    : "Masa/Otaq";
    ...
    <div className="basket-page">
      <div className="container">
        ...
        <div className="table-info" aria-label="context-number">
          <i
            className={`icon fas ${
              displayIsTable ? "fa-chair" : "fa-door-open"
            }`}
            aria-hidden="true"
          ></i>
                    ...
                    {relatedProducts.map((p) => (
                      <div key={p.id} className="suggestion-card">
                        ...
                        <div className="sugg-info">
                          <h3 className="sugg-name" title={p.name_az}>
                            {p.name_az}
                          </h3>
                          <div className="sugg-price">
                            {Number(p.cost).toFixed(2)} AZN
                          </div>
                          <button
                            className="sugg-add"
